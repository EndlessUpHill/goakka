...
func main() {
	...
	// Create Redis broker (connect to Redis on localhost:6379)
	redisBroker := redis.NewRedisBroker("localhost:6379")
	...
	supervisor := core.NewSupervisor(ctx, redisBroker)
	...
	childSupervisor := core.NewSupervisor(childCtx, redisBroker)
